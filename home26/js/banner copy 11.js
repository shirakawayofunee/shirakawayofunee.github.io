...
gsap.fromTo(".backstory-left", 
    { x: "-30vw" }, 
    { x: "0", ease: "none", 
      scrollTrigger: {
          trigger: "#slide5",
          start: "top 90%", // 核心：当 slide5 顶部到达视口 90% 位置（即刚好漏出10%）
          end: "top 0%",    // 当 slide5 顶部完全触碰视口顶部
          scrub: true       // 动画严格跟随鼠标滚轮进度
      }
    }
);
...
ScrollTrigger.create({
    trigger: "#slide5",
    start: "top top",
    end: "+=150%",     // 钉住的持续滑动距离（可根据文字阅读量调大调小）
    pin: true,
    pinSpacing: false  // 【极其关键】：必须为 false！这样 slide6 滚上来时才会像“盖被子”一样盖在 slide5 上，而不是把它顶走。
});


// ==================== 3. 退场动画 (由 slide6 盖上来时触发) ====================
gsap.to("#slide5 .pc", {
    scale: 0.85,             // 微微后退缩小
    y: "-50vh",              // 微微向上移动
    filter: "brightness(0.1)", // 【核心魔法】直接修改明度！从 1 降到 0.1，实现完美的“被阴影笼罩”效果
    opacity: 0,              // 最终变完全透明，确保不会挡住下一屏的点击事件
    ease: "power1.inOut",
    scrollTrigger: {
        trigger: "#slide6",
        start: "top 80%",    // 当 slide6 盖住屏幕 20% 时，开始变暗缩小
        end: "top 10%",      // 当 slide6 快要完全占满屏幕时，彻底退场完毕
        scrub: true
    }
});