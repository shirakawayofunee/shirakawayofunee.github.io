const connections = [
  ...
  /*     { from: 4, to: 0, label: "敬慕の愛、利用", color: "#b2160b",       side: "right",
  sideIn: "left",
  slot: 1, }, */
  ...
  { 
    from: 4, to: 5, label: "反逆", color: "#b2160b", dashed: true,
    side: "top", sideIn: "left", fromSlot: 0, toSlot: -1, bendOffset: 140 
  }
  ...
  /*       { 
    ...
    { 
      from: 0, to: 8, label: "探り、GB、女攻め男受け", color: "#b2160b", dashed: true,
...
function drawLines() {
  ...
  connections.forEach((conn) => {
    ...
    // 如果你有旧的 slot 参数，将其映射给 fromSlot 和 toSlot
...
function createLabel(text, x, y, strokeColor) {
  ...
  textEl.setAttribute("font-size", isMobile ? "10px" : "12px");
  ...
  // 测量并绘制背景框
  document.body.appendChild(textEl); // 临时加入以测量
  const bbox = textEl.getBBox();
  document.body.removeChild(textEl);

  const rect = document.createElementNS("http://www.w3.org/2000/svg", "rect");
  rect.setAttribute("x", x - bbox.width / 2 - paddingH);
  rect.setAttribute("y", y - bbox.height / 2 - paddingV); // 修正垂直居中偏移
  ...
  textEl.setAttribute("y", y - bbox.height / 2 + (isMobile ? 5 : 6));
...
function createLabel(text, x, y) {
  ...
  textEl.setAttribute(
    "font-size",
    window.innerWidth <= 576 ? "9.5px" : "12.5px"
  );
