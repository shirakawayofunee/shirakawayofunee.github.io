...
// ======================================================
// 3. 核心退场：Slide6 顶上来，Slide5 内容变暗并上移
// ======================================================
const exitTl = gsap.timeline({
    scrollTrigger: {
        ...
        scrub: true,
        invalidateOnRefresh: true,
        // 当 slide6 离开时，强制刷新一次层级状态
        onToggle: self => {
            if (!self.isActive) {
                // 防止回滚到边界时遮罩不消失的保底逻辑
                if (self.progress === 0) gsap.set(".backstory-mask", { opacity: 0 });
            }
        }
    }
});
...
exitTl
    ...
    .fromTo("#slide5 .pc", 
        ...
        { 
            scale: 0.85, 
            y: "-40vh", // 位移稍微减小一点，配合 slide6 的推入感
            ease: "none" 
        }, 
        0
    );
...
// ======================================================
// 4. 层级保底 (防止 GSAP Pin 自动改写 z-index)
// ======================================================
// 这一步确保在滚动过程中，#slide6 永远在 #slide5 的上面
ScrollTrigger.create({
    trigger: "#slide6",
    start: "top 100%",
    onEnter: () => { 
        gsap.set("#slide6", { zIndex: 20 });
        gsap.set("#slide5", { zIndex: 5 });
    },
    onLeaveBack: () => {
        gsap.set("#slide6", { zIndex: 10 });
    }
});