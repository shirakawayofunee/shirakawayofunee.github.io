...
window.timelineMsg = [
  ...
  },
  {
    ...
    isMajor: true, 
    customRow: 8,
        // 【核心设定】因为前一个图片宽度是 500px：
    // 如果设置为 -500，则该文本的左边缘与图片的左边缘【完全垂直对齐】
    // 如果设置为 -460，则文本会比图片稍微往右错开 40px，显得更有层次感
    customGap: -460, 
  },
  {
    date: "N.F.21年",
...
function generateLayout(data) {
  ...
  const placedItems = []; // 存储已经放置好的元素，用于物理碰撞检测
  ...
  return data.map((item, index) => {
    ...
    if (prevItem) {
      ...
      if (!isOverlappingVertically) {
        // A) 垂直方向【不重叠】：可以使用负间距，允许它们横向错开、层叠（部分希望重叠）
        if (item.forceCompact) {
          baseGap = -200; // 强制极度紧凑（多用于图片和紧接着的元素错开）
        } else if (item.type === "image" && prevType === "image") {
          baseGap = -prevItem.width + 50; // 图片横向错开交叠
        } else if (item.type !== "image" && prevType !== "image") {
          baseGap = -80;  // 文字块横向错开
        } else {
          baseGap = -30;  // 默认错开时的微调
        ...
        // B) 垂直方向【有重叠】：初始时就给予正向间距（部分希望紧贴，部分希望留白）
        if (prevItem.isMajor) {
          baseGap = 120; // 避让大事件
        } else if (item.isMajor) {
          baseGap = 150; // 大事件自身边距
        } else if (item.forceCompact) {
          baseGap = 20;  // 强制紧凑但有垂直重叠，留极小安全距离（紧贴）
        } else {
          baseGap = 60;  // 普通元素正常留白
        }
      }
    ...
    // 计算初步 X 坐标
    let myX = (index === 0) ? X_START : (prevItem.x + baseGap);

    // --- 4. 2D 物理碰撞检测与动态推开 ---
    let collided = true;
    let safetyCounter = 0; // 防死循环安全阀
    ...
    while (collided && safetyCounter < 150) {
      ...
      for (let i = 0; i < index; i++) {
        ...
        if (vertOverlap) {
          // 2. 如果垂直重合，获取该前置元素要求的“右侧保护边界（marginRight）”
          let marginRight = 60; // 默认留几十像素
          
          if (prev.isMajor) {
            marginRight = 120; // 大事件后方空出更多距离
          } else if (prev.forceCompact) {
            marginRight = 20;  // 紧贴元素后方只留极少像素
          } else if (prev.type === "image") {
            marginRight = 50;  // 图片后方预留中等距离
          }

          const obstacleRightBoundary = prev.x + prev.width + marginRight;

          // 3. 如果当前 myX 闯入了前置元素的保护边界，则发生碰撞，将其强行推开
          if (myX < obstacleRightBoundary) {
            myX = obstacleRightBoundary;
            collided = true; // 标记发生了推移，需要重新检验（防止推开后又撞上其他行的旧元素）
            break; 
          }
        }
      }
