...
/*     { from: 0, to: 6, label: "元同僚", color: "#b2160b", dashed: true },
    ...
    { 
      ...
      label: "描述", 
      // --- 更加精细的控制 ---
      side: "right",     // 从 L 的右边出发
      sideIn: "left",    // 进入 Severo 的左边
      fromSlot: 1,       // 在 L 右侧排队的位置（比如最上面）
      toSlot: -2,        // 在 Severo 左侧排队的位置（比如最下面）
      bendOffset: 60     // 转接头离 L 的距离
    }
...
connections.forEach((conn) => {
  ...
  // === 计算精确起点 (sX, sY) ===
  let sX = p1.x, sY = p1.y;
  if (conn.side === 'left') { sX -= p1.radius; sY += fSlot * step; }
  else if (conn.side === 'right') { sX += p1.radius; sY += fSlot * step; }
  else if (conn.side === 'top') { sY -= p1.radius; sX += fSlot * step; }
  else { sY += p1.radius; sX += fSlot * step; }

  // === 计算精确终点 (eX, eY) ===
  let eX = p2.x, eY = p2.y;
  const margin = 15; // 箭头余量
  if (conn.sideIn === 'left') { eX -= (p2.radius + margin); eY += tSlot * step; }
  else if (conn.sideIn === 'right') { eX += (p2.radius + margin); eY += tSlot * step; }
  else if (conn.sideIn === 'top') { eY -= (p2.radius + margin); eX += tSlot * step; }
  else { eY += (p2.radius + margin); eX += tSlot * step; }

// ==================== 4. 关键：直线与折线的条件路由逻辑 ====================
let pathD, lx, ly;
    
// 直线判定阈值（单位：像素），如果差异小于这个值，则判定为一条直线
const straightThreshold = 5; 

// === 情况A：纯水平直线 ===
// 判定条件：起点和终点都在同一水平线 (sY 和 eY 几乎相等)，且出发/进入方向正确
if (Math.abs(sY - eY) < straightThreshold && (side === 'left' || side === 'right')) {
  pathD = `M ${sX} ${sY} L ${eX} ${sY}`;
  lx = (sX + eX) / 2; // 标签在直线中间
  ly = sY;
} 
// === 情况B：纯垂直直线 ===
// 判定条件：起点和终点都在同一垂直线 (sX 和 eX 几乎相等)，且出发/进入方向正确
else if (Math.abs(sX - eX) < straightThreshold && (side === 'top' || side === 'bottom')) {
  pathD = `M ${sX} ${sY} L ${sX} ${eY}`;
  lx = sX;
  ly = (sY + eY) / 2; // 标签在直线中间
} 
// === 情况C：标准折线（Z字形） ===
...
path.style.strokeLinejoin = "round"; // 保持圆角效果（对直线无影响）
