...
const sections = document.querySelectorAll('.gsap-section'); // 你的9个 slide 必须都有这个 class
...
function changeBanner(num) {
    ...
    // === 新增：GSAP 滚动到对应 section ===
    const targetSection = sections[num];   // sections 是你之前定义的 .gsap-section 数组
    if (targetSection) {
        gsap.to(window, {
            duration: 0.8,           // 切换速度，可调
            scrollTo: {
                y: targetSection,
                offsetY: 0          // 需要的话可以加 offsetY: 100
            },
            ease: "power2.inOut",
            onComplete: () => {
                activeIndex = num;   // 强制同步状态
            }
        });
    }
