  ...
  const connections = [
    // L (0) 和 Severo (1) 的三条线 —— 同一水平线，纯水平平行（按你指定的视觉顺序）
  ...
  function drawLines() {
    ...
    pairMap.forEach((pairConns, pairKey) => {
      ...
      const isHorizontalPair = Math.abs(dy) < 25; // 是否同一水平线

      // === 特殊处理：L-S 三条线且同一水平线（纯水平平行）===
      if (pairConns.length === 3 && isHorizontalPair) {
        // 按视觉顺序排序：最上 L→S → 中间客观 → 最下 S→L
        pairConns.sort((a, b) => {
        ...
        pairConns.forEach((conn, index) => {
          const offset = (index - 1) * baseGap; // -30 / 0 / +30
      ...
      // === 普通情况：1条或2条线，使用正交 Z 字型 ===
      pairConns.forEach((conn, subIndex) => {
        const gap = window.innerWidth <= 576 ? 14 : 50;
