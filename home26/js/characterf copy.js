let characterMsg = [
  {
    ...
    check: "1c.png",
    title: "1t.png",
    // 原 str 字段保留，但不再使用；你可以逐步迁移内容到新字段
    str: `...`, // 原内容
    // 新增字段：分块内容（你自行填充）
    str1: `能力者、178cm。<br>犯罪組織の<span>主宰者</span>であり、部下たちから恐れられ、崇拝されている存在。
    <br>年齢と比べてあまりにも<span>若々しい顔立ち</span>、過度に白い肌色――虞美人の時間は、遠い遠い昔に完全に止まってしまったかのようだ。
    実年齢不明、60>？？>36。<br>艶やか、常に世を拗ねるような微笑を浮かべている。`,
    str2: `好き嫌いが激しく、享楽主義者で実用も重んじる、細やかで優しい一面も持っている。<br>落ち着き払った自信に満ちていて、自信かつ優雅な態度を保ち、性格は八方巧み。夹缝中的最强者，将上庭与地底的斗争视作狗咬狗。当前的目标是在自己死前，给下属们寻求新的安全居所。`,
    str3: `▍人間単体の最強戦力<br>核兵器＞＞＞L≈S級能力者＞使徒≥軍隊＞一般能力者＞＞力強いたくましい人間＞一般人<br>▍メインストーリー（ネタバレ？）<br>▍好きなもの<br>性格……孤高でそっけなく、決断力と行動力がある、表で暴虐且つ無慈悲だが、裏は温情に富んで、特に部下に対して。<br>逆鱗……花园<br>得意なこと……「栽培」、死の芸術、美学<br>趣味……、<br>好き……花<br>嫌い……自由が制限されること、無意味な犠牲<br>特技……開花、癒し`,
    str4: `▍背景<br>Lは幼少期、軍人である両親が前線で命を落としました。<br>その命の価値のない犠牲は「崇高な使命のため」とされましたが、彼女にとってその言葉は無意味で、むしろ規則の虚偽と束縛を感じさせるものでした。<br>両親の死は彼女に反抗心を芽生えさせ、彼女は規則に縛られることを拒み、命を代償に禁断の力と契約を結びました。<br>その力を使って、他の人々が規則から解放され、彼女のように自由に生きる手助けをすることを決意します。<br>▍口調<br>戏谑的で冷酷な口調<br>▍会話例<br>「命を大切に思うことには賛成よ。どんな醜く卑劣な人間でも、美しい花を咲かせられる。肉体も魂も養分となって、花を育て、開花するまで。それこそが本当の純粋な命だ。」<br>...（剩余对话）`,
    // 新增字段：每个标签页的标题（你自行决定）
    tabTitle1: "基本信息",
    tabTitle2: "个性特征",
    tabTitle3: "战斗与喜好",
    ...
    },
    {
      ...
      check: "7c.png",
      title: "7t.png",
      str: `独立軍第二代総帥、新城邦初の民選議長、FACの創設者であり初代総司令――デレヴァン・スティール。<br>
  ...
  function changeCharacter(num) {
    ...
    for (let i = 1; i <= 4; i++) {
      ...
      if (content) {
        ...
        btn.show().find('img').attr('alt', title); // 更新按钮 alt 为标题（或用图片显示标题）
      } else {
        $(panelId).empty();
        btn.hide(); // 隐藏空标签页
      }
