let queryMobile = window.matchMedia(
...
var swiperAllM = new Swiper("#allSwiperMobile", {
  ...
  on: {
    slideChange: function () {
      //changeBannerMobileIndex(this.activeIndex); // index.html
      console.log(this.activeIndex, 12344);
      ...
      if (this.activeIndex === 4) {
        $(".down-btn-mobile").show();
      }
      changeBannerMobile(this.activeIndex); // detail.html/video.html
...
function changeStatementMobile(type) {
  ...
  if (type) {
    if ($(".down-btn-mobile").attr("data-customShow") === "0") {
      $("#allSwiperMobile").css({
      ...
      $(".down-btn-mobile").attr("data-customShow", "1");
      $(".head-mob").css({
        transform: `translate(0,${minMobile})`,
        transition: "all 0.3s",
      });
    } else {
      $("#allSwiperMobile").css({
    ...
    $(".down-btn-mobile").attr("data-customShow", "0");
    swiperAllM.enable();
    $(".box-egg-mobile").hide();
  }
}
...
// 预加载live2D gif图
function changeBannerMobileIndex(url) {
  // if ($(".gifUrl").length) {
  //   return;
  // }
  $(".gifUrl").attr("src", `./img/role/${url}`);
  // $("#cont-gif-img").append(`
  //   <img src="./img/role/${url}" class="characterMobile-top-role characterFadeInRight gifUrl" alt=""
  //     ></img>
  //   `);
}

function isMobileBackstoryScroll() {
  var element = $("#backstoryMobileStr");
  var elementChild = $("#backstoryMobileStr-cont");
  console.log(element.height(), element.innerHeight(), elementChild.height());
  if (element.height() < elementChild.height()) {
    console.log("出现了滚动条");
    $("#backstoryMobileStr").on("touchstart", function () {
      $(".backstoryMobile").addClass("swiper-no-swiping");
    });
    $("#backstoryMobileStr").on("touchend", function () {
      $(".backstoryMobile").removeClass("swiper-no-swiping");
    });
  } else {
    $("#backstoryMobileStr").on("touchstart", function () {
      $(".backstoryMobile").removeClass("swiper-no-swiping");
