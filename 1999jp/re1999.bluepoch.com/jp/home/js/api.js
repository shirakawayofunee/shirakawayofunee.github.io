...
window.onload = gameMsg(gameModelData).then((data) => {
  initFooter().then(function () {
    ...
    $(".app-store-link").prop("href", data.pageData[0].iOSDownloadUrl);
    $(".pageVideo").data("video",data.pageData[0].youtubeVideoId)
    $(".google-pay-link").prop("href", data.pageData[0].androidDownloadUrl);
    $(".pc-load-link").prop("href", data.pageData[0].doubleEndUrl);
    $(".mumu-load-link").prop("href", data.pageData[0].mumuDownloadUrl);
...
function initFooter() {
  return new Promise((resolve, reject) => {
    ...
    platform === "mobile"
      ? $(".statementMobile").load("statementMobile.html", function () {
          resolve();
        })
      : $(".statement.pc").load("statementPC.html", function () {
          resolve();
        });
  });
}

function initCookiePop(){
  $(".sl-cookie-pop").load("cookie.html", function () {
    !localStorage.getItem('jp_home_cookie') && $('.sl-cookie-pop').show()
  })
}
function handleCoolieHidden(){
  localStorage.setItem('jp_home_cookie',true)
  $('.sl-cookie-pop').hide()
}